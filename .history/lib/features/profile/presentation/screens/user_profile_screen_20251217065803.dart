          ...
          child: Column(
            ...
            children: [
              ...
              _buildProfileButton(
                ...
                label: 'Logout',
                iconAsset:
                    AppAssets.download, // Or use a logout icon if available
                onPressed: () async {
                  // 1. Call the provider to sign out
                  // (This already removes 'is_logged_in' from storage)
                  await auth.signOut();

                  if (!context.mounted) return;

                  // 2. Clear the entire navigation stack and go to AuthWrapper
                  // This forces the AuthWrapper to run its initState check again
