...
class _SignUpScreenState extends State<SignUpScreen> {
  ...
  // ✅ FIXED: Logic moved to a separate method to fix context/mounted errors
  Future<void> _handleSignUp() async {
    ...
    // Guard 1: Check mounted after first await
    if (!mounted) return;

    if (!success) {
      final message = auth.errorMessage ?? 'Failed to create account';
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text(message)));
    } else {
      // 2. Update Username
      await auth.updateUsername(displayName: nameController.text.trim());

      // Guard 2: ✅ Added missing check after second await
      if (!mounted) return;
  ...
  Widget build(BuildContext context) {
    ...
    var signInBg = AppAssets.signInBg;
    return Scaffold(
          ...
          Positioned(
            ...
            child: Container(
              ...
              child: ElevatedButton(
                ...
                // ✅ FIXED: Calls the local method instead of defining async inline
                onPressed:
